...
class AuthorListViewTest(TestCase):
    ...
    def test_view_url_exists_at_desired_location(self):
        resp = self.client.post('/catalog/authors/', follow=True)
        print(f'\nresp /catalog/authors/: {resp}\n')
        self.assertEqual(resp.status_code, 200)

    def test_view_url_accessible_by_name(self):
        resp = self.client.get(reverse('authors'), follow=True)
        print(f"\nreverse('authors'): {reverse('authors')}")
        print(f'\nself.client.get(reverse(\'authors\')): {resp}\n')
        self.assertEqual(resp.status_code, 200)
    ...
    def test_pagination_is_ten(self):
        ...
        self.assertTrue( len(resp.context['author_list']) == 13)

    def test_lists_all_authors(self):
        #Get second page and confirm it has (exactly) remaining 3 items
        resp = self.client.get(reverse('authors')+'?page=2', follow=True)
        self.assertEqual(resp.status_code, 200)
        self.assertTrue('is_paginated' in resp.context)
        self.assertTrue(resp.context['is_paginated'] == True)
...
class LoanedBookInstancesByUserListViewTest(TestCase):
    ...
    def test_redirect_if_not_logged_in(self):
        ...
        print(resp)
        print('/accounts/login/?next=/catalog/mybooks/')
